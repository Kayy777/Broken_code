...
if __name__ == '__main__':
    ...
    while winner == 0 and moves_exist(game):
        ...
        available = False
        while not available:
            row = convert_input_to_coordinate(int(input("Which row? (start with 1, on first round) ")))
            column = convert_input_to_coordinate(int(input("Which column? (start with 1,on first round) ")))
            available=check_space_empty
